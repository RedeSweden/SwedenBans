...
public class Msg
{
	...
	public static void reload() {
        final File f = new File(SwedenBans.instance.getDataFolder(), "messages.yml");
        Msg.cfg = new YamlConfiguration();
        try {
            final YamlConfiguration defaults = new YamlConfiguration();
            InputStream in = SwedenBans.instance.getResource("messages.yml");
            defaults.load(in);
            in.close();
            if (f.exists()) {
                Msg.cfg.load(f);
                ...
                in = SwedenBans.instance.getResource("messages.yml");
            ...
            try {
                Msg.cfg.load(SwedenBans.instance.getResource("messages.yml"));
            }
            catch (Exception ex) {
                ex.printStackTrace();
            }
    ...
    public static String get(final String loc, final String[] keys, final String[] values) {
        ...
        if(loc.startsWith("disconnection") || loc.startsWith("announcement")) {
            msg = String.format("§§\n &c&lSWEDEN PUNISHMENTS \n§§\n%s", msg);
            msg = String.format("%s\n§§", msg);
        }
